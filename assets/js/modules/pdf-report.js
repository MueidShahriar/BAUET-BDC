...
export function createMonthlyReportDownloader({
  ...
  async function downloadMonthlyReportPdf(triggerEl) {
    ...
    try {
      ...
      const { monthGroups, undated } =
        groupRecentDonationsByMonth(donationEntries);
      const monthCounts = monthGroups.map((group) => group.length);
      const totalDonations = donationEntries.length;
      const countedByMonths = monthCounts.reduce(
      ...
      const getEntryEstimatedHeight = (donation = {}) => {
      ...
      const ensureEntrySpace = (estimatedHeight = 24) => {
      ...
      const writeWrappedText = (text, indent = 0, lineGap = 6) => {
        const segments = doc.splitTextToSize(
          String(text),
          contentWidth - indent,
        );
      ...
      const writeInlineSegments = (segments, indent = 0, lineGap = 5) => {
        ...
        segments.forEach((segment) => {
          ...
          const color = Array.isArray(segment.color)
            ? segment.color
            : palette.text;
      ...
      const drawReportHeader = () => {
        ...
        doc.text(
          `Generated on ${_pR(now.getDate())}/${_pR(now.getMonth() + 1)}/${now.getFullYear()}`,
          marginX + 4,
          cursorY + 13,
        );
        cursorY += 16;
      ...
      if (sortedYears.length > 0) {
        ...
        sortedYears.forEach((year, idx) => {
          ...
          doc.text(
            String(year),
            tableStartX + monthColWidth + idx * colWidth + colWidth / 2 - 10,
            cursorY,
          );
        });
        cursorY += 6;
      ...
      sortedYears.forEach((year) => {
        ...
        yearDonations.forEach((entry) => {
          ...
          const dateObj = rawDate ? new Date(rawDate) : null;
          if (dateObj && !Number.isNaN(dateObj.getTime())) {
            monthGroupsForYear[dateObj.getMonth()].push({
              donation: entry,
              dateObj,
            });
          }
        ...
        for (let idx = monthGroupsForYear.length - 1; idx >= 0; idx -= 1) {
          ...
          const firstEntryHeight = getEntryEstimatedHeight(
            firstEntry?.donation,
          );

          drawMonthHeader(
            chartLabels.months[idx],
            String(year),
            entries.length,
            firstEntryHeight,
          );
      ...
      if (!printedAnyDetails && !undated.length) {
        writeWrappedText(
          "No donation entries are available for the selected period.",
          0,
          6,
        );
      }
      ...
      showModalMessage(
        "success-modal",
        "Unable to generate monthly report. Please try again.",
        "Error",
      );
    } finally {
      if (triggerEl) {
